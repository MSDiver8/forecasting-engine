...
from exceptions.exceptions import Real_Forecast_Error, Psevdo_Forecast_Error
...
def ps_RWDS_forecast(Data: pd.DataFrame,
    ...
    # добавить if none для сезоности
...
def TS_real_forecast(Data: pd.DataFrame,            
    ...
    if Window_in_years == None or Window_in_years == 0:
        ...
        forecast_list = [round(alpha + (df[-1:].index.to_list()[0] + i) * betta, 2) for i in range(Forecast_horizon)] 
